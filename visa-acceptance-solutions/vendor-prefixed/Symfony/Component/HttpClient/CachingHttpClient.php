...
class CachingHttpClient implements HttpClientInterface, ResetInterface
{
     ...
     */
    private const EXCLUDED_HEADERS = ['connection' => true, 'keep-alive' => true, 'proxy-authenticate' => true, 'proxy-authentication-info' => true, 'proxy-authorization' => true, 'te' => true, 'trailer' => true, 'transfer-encoding' => true, 'upgrade' => true];
    /**
    ...
    public function request(string $method, string $url, array $options = []): ResponseInterface
    {
        ...
        $passthru = function (ChunkInterface $chunk, AsyncContext $context) use ($expiresAt, $fullUrlTag, $requestHash, $varyKey, $varyFields, &$metadataKey, $cachedData, $freshness, $url, $method, $options, $hasClientConditionalValidator, $sentCacheValidator, $allowStaleFallback): \Generator {
            ...
            if ($chunk->isFirst()) {
                ...
                if (304 === $statusCode && null !== $freshness) {
                    ...
                    $updatedCachedData = array_replace($cachedData, ['stored_at' => $responseTime, 'initial_age' => $correctedInitialAge, 'headers' => $updatedHeaders]);
                    ...
                    $updatedMetadataIsCacheable = !\in_array('*', $newVaryFields, true) && $varyFields === $newVaryFields && $this->isServerResponseCacheable($cachedData['status_code'], $options['normalized_headers'], $updatedCachedData['headers'], $updatedCacheControl);
            ...
            if ($chunk->isLast()) {
                ...
                $this->cache->get($metadataKey, static function (ItemInterface $item) use ($context, $headers, $maxAge, $expiresAt, $fullUrlTag, $metadataKey, $attemptTag, $firstChunkKey, $responseTime, $correctedInitialAge): array {
                    $item->tag([$fullUrlTag, $metadataKey, $attemptTag])->expiresAt($expiresAt);
                    return ['status_code' => $context->getStatusCode(), 'headers' => self::filterStorableHeaders($headers), 'initial_age' => $correctedInitialAge, 'stored_at' => $responseTime, 'expires_at' => self::calculateExpiresAt($maxAge), 'next_chunk' => $firstChunkKey];
                }, \INF);
                yield $chunk;
                return;
            }
    ...
    private function evaluateCacheFreshness(array $data): Freshness
    {
        ...
        if (isset($cacheControl['must-revalidate']) || $this->sharedCache && (isset($cacheControl['proxy-revalidate']) || self::hasValidSharedMaxAge($cacheControl))) {
            return Freshness::MustRevalidate;
        }
        if (null !== ($staleIfError = self::parseDeltaSeconds($cacheControl['stale-if-error'] ?? null)) && $now - $expires <= $staleIfError) {
    ...
    private function isServerResponseCacheable(int $statusCode, array $requestHeaders, array $responseHeaders, array $cacheControl): bool
    {
        ...
        if ($this->sharedCache) {
            if (!isset($cacheControl['public']) && !self::hasValidSharedMaxAge($cacheControl) && !isset($cacheControl['must-revalidate']) && isset($requestHeaders['authorization'])) {
                return false;
            }
    ...
    {
        return !isset($cacheControl['no-cache']) && !isset($cacheControl['no-store']) && isset($headers['last-modified']);
    }
    /**
    ...
    private function createResponseFromCache(array $cachedData, string $method, string $url, array $options, string $metadataKey, \DateTimeImmutable|false|null $newExpiresAt = false): MockResponse
    {
        ...
        return MockResponse::fromRequest($method, $url, $options, new MockResponse($body(), ['http_code' => $cachedData['status_code'], 'response_headers' => ['age' => $this->getCachedResponseAge($cachedData)] + $cachedData['headers']]));
    }
    ...
    {
        return MockResponse::fromRequest($method, $url, $options, new MockResponse('', ['http_code' => 304, 'response_headers' => array_diff_key($headers, self::EXCLUDED_HEADERS)]));
    }
}