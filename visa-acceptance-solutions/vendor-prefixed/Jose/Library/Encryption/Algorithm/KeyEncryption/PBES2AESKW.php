...
{
    public const DEFAULT_MAX_COUNT = 1000000;
    public function __construct(private readonly int $salt_size = 64, private readonly int $nb_count = 4096, private readonly int $max_count = self::DEFAULT_MAX_COUNT)
    {
    ...
    protected function checkHeaderAdditionalParameters(array $header): void
    {
        ...
        if ($header['p2c'] > $this->max_count) {
            throw new InvalidArgumentException(sprintf('The header parameter "p2c" is too large. The maximum allowed value is %d.', $this->max_count));
        }
    }
